...
if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
	...
	if ( ! class_exists( 'SendleAPI')){
		...
		class SendleAPI {
			 ...
			 * @return 
			 */ 
			function sendle_select_weight($weight){
				switch($weight){
					case ($weight<=0.5):
						return 0.5;
						break;
					...
					if(isset($operation)&&isset($order_id)){
						...
						switch($operation){
							...
							case "book":
								...
								if(get_option('woocommerce_weight_unit') == "kg"){
									...
									foreach ($order->get_items() as $item){
										...
										if ( ! $_product->is_virtual() ) {
											$cart_weight += $_product->get_weight() * $item['qty'];
										}
								...
								$result['pickup']['weight'] = $this->sendle_select_weight($cart_weight);
								$result['pickup']['weight_actual'] = $cart_weight;
			...
			public function error_handler($curl_input, $curl_info){
				...
				switch ($curl_info['http_code']){
					...
					case 402:
						$result['success'] = false;
						$result['output'] = $curl_result;
						$result['header'] = "<h3>Account Error</h3><p>Please review and resolve the messages below before re-submitting the form.</p>";
						$result['msg'] .= "<p><b>Error: </b>".$result['output']['error']."<br>";
						$result['msg'] .= $result['output']['error_description']."</p>";
						if(isset($result['output']['messages'])){
							$result['msg'] .= "<p><h4>Details:</h4></p>".$curl_input;
